...
const PATH = 'studio-multiagente/workflows/agent_municipal_precheck.json';
const json = JSON.parse(await readFile(PATH, 'utf-8'));

const payload = { name: json.name, nodes: json.nodes, connections: json.connections, settings: json.settings || { executionOrder: 'v1' } };

console.log('[deploy] POST /api/v1/workflows');
const r = await fetch(`${N8N_API_URL}/api/v1/workflows`, {
  method: 'POST', headers: { 'X-N8N-API-KEY': N8N_API_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
});
const d = await r.json();
if (!r.ok) { console.error('FAIL', r.status, JSON.stringify(d, null, 2)); process.exit(1); }
console.log('[deploy] CREATED id=', d.id);

const ra = await fetch(`${N8N_API_URL}/api/v1/workflows/${d.id}/activate`, {
  method: 'POST', headers: { 'X-N8N-API-KEY': N8N_API_KEY }
});
const da = await ra.json();
if (!ra.ok) { console.error('ACTIVATE FAIL', ra.status, JSON.stringify(da, null, 2)); process.exit(1); }
console.log('[deploy] ACTIVE=', da.active);

json._n8n_id = d.id;
await writeFile(PATH, JSON.stringify(json, null, 2) + '\n', 'utf-8');
console.log('[deploy] DONE. id=', d.id);
