...
- **Fase 2 parcial ✅ (2026-04-26)**: `cron_invoice_approval_followup` (`xM7YlAGwbbgbFaGI`, diario 11:30) detecta facturas pending_review >5d + approved sin pagar >30d + disputed >14d. Email digest con total_amount_pending para priorizar.
...
- **Fase 2 parcial ✅ (2026-04-26)**: endpoint público para clientes ya existe (`aftercare_public_form` `WHtdrr3tJpei3IM8` + `aftercare_public_submit` `x5j2VKbz9tfQyqzl`). `cron_aftercare_sla_breach` (`U5hvcNLQGOrbyQ6J`, diario 08:30) detecta incidentes `assigned`/`in_progress` que llevan más días asignados del SLA por severidad (urgent=2d, high=5d, medium=14d, low=30d) — sin resolver por el colaborador o gremio responsable.
- **Pendiente (fase 2)**: auto-asignación según mapa trade→contacto preferente; auto-escalado tras SLA breach (no solo email digest); comparación visual evidencia inicial vs final con Vision.

### 3.7 ~~Plan de seguridad y salud~~ — ✅ CONSTRUIDO (2026-04-25)
**Movido a sección 1.2 como `agent_safety_plan` operativo.**

### 3.8 ~~Control de calidad in-situ~~ — ✅ MVP CONSTRUIDO (2026-04-25/26)
- **Construido (fase 1)**: tabla `qc_checks` (migración 018) con jsonb items (id/label/description/status/comment/evidence_url) y trigger que auto-recalcula status según items (open/in_progress/complete/blocked/cancelled). Workflows `qc_generate` (`ge3Do1cEeSDuCtzk`) genera checklist desde template por fase, `qc_complete` (`JTPN78VZtz8i0ZwB`) actualiza un item.
- **Fase 2 ✅ UI móvil (2026-04-26)**: `qc_public_form` (`Pqod9AyvG0opCrLU`) sirve un form HTML responsive mobile-first vía GET `/webhook/qc-form?qc_id=<uuid>` con header x-api-key. 3 botones grandes pass/fail/skip + textarea + autosave via fetch al endpoint qc-complete. Diseñado para visita a obra desde el móvil.
- **Fase 2 ✅ vigilancia (2026-04-26)**: `cron_qc_review` (`beICCi9A5WYU5w45`, diario 09:15) alerta checklists `blocked` o sin progreso >7d. `cron_qc_handover_to_acta` (`QNp8QK9x7XFehSs6`, diario 11:00) cierra ciclo QC `recepcion_provisional` complete → `acta_recepcion_provisional` firmada → CFO.
- **Pendiente (fase 2)**: app nativa (vs form HTML) para fotos directas; vinculación foto→item con upload Drive; comparación temporal entre checks de la misma fase en proyectos análogos vía memory similarity.
...
- **Fase 2 parcial ✅ (2026-04-26)**: `cron_contract_followup` (`ZlzJpRwOnoG1altD`, diario 09:45) alerta contracts esperando firma >7d + drafts olvidados >14d + expirados sin marcar. Y `cron_proposal_to_contract` (`1B6SNirRuyQdsY9U`, diario 10:30) detecta proposals aprobadas sin contract `encargo_profesional` activo y sugiere generar el contrato. `cron_qc_handover_to_acta` (`QNp8QK9x7XFehSs6`, diario 11:00) cierra el ciclo QC `recepcion_provisional` → acta firmada.
- **Pendiente (fase 2)**: integración DocuSign/FNMT/Autofirma (firma electrónica con hash); plantillas configurables en BD (`contract_templates`); generación PDF automática; hook agent_proposal aprobada → auto-genera encargo (en lugar de sugerir).
...
- **Fase 2 parcial ✅ (2026-04-26)**: hook `agent_briefing` ↔ `pathology_findings` activo: el SQL de Load Project + Client del agent_briefing inyecta el array de findings unresolved (`status NOT IN repaired/dismissed`) ordenado por severity. `cron_pathology_review` (`tFYGrFmo3zBwirre`, diario 12:00) alerta findings stale >30d sin actualizar + criticos/affects_safety unresolved.
- **Pendiente (fase 2)**: hook con `agent_costs` para sumar `estimated_intervention_cost` como partida del breakdown; `pathology_confirm/repair` workflows con webhook (cambiar status detected→confirmed→in_repair→repaired); RAG semántico sobre catálogos técnicos de patología; collab habitual de la especialidad sugerido automáticamente.
...
- **Fase 2 ✅ (2026-04-26)**: `cron_collab_review` (`sJpNiWYCIlCvqB5i`, diario 10:00) detecta 3 condiciones (deadline vencido + delivered>7d sin approved + invited>5d sin respuesta) y manda email digest agrupado.
- **Pendiente (fase 2)**: RLS multi-tenant cuando sea multi-estudio; portal colaborador (link mágico para aceptar/rechazar/entregar sin email); hook agent_pathology → propone collab habitual de la especialidad; auto-invoice tras `approved`.
...
- **Fase 2 ✅ (2026-04-26)**: `cron_compliance_audit_weekly` (`tTZaWj86DPW0TNRt`, domingos 08:00) hace HTTP loop sobre proyectos activos llamando a `/webhook/audit-project` con send_email=false; agrega scorecards y manda digest si has_alerts (grade<B o critical>0). Y `util_admin_compliance_overview_html` (`jxCwiphu6ahrBNXv`) sirve un dashboard HTML con grade A/B/C/D + 11 checks por proyecto en una sola pantalla, sin LLM (deterministico instant) — para vista panorámica del estudio en cualquier momento.
- **Pendiente (fase 2)**: calibración del scoring con feedback real; hook compliance fail crítico → notificación inmediata (vs digest semanal); export del scorecard a PDF firmable.
