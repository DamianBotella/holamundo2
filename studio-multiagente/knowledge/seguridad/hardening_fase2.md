...
3. ~~**Cifrar `client_conversations.question/answer`**~~ ✅ Hecho 2026-04-26 (migration 031, mismo patrón que migration 009: columnas `_enc bytea` + trigger BEFORE INSERT/UPDATE auto-cifra con `pii_encrypt()`. Backward-compatible: workflows actuales no requieren cambio). **Lector actualizado (migration 032)**: la vista `gdpr_client_data_view` ahora usa `pii_decrypt(cc.question_enc)` y `pii_decrypt(cc.answer_enc)` — preparada para el futuro DROP de las columnas plain.
4. ~~**Cifrar `gdpr_requests.details`**~~ ✅ Hecho 2026-04-26 (migration 031, idem). Round-trip verificado (`pii_decrypt(pii_encrypt(x)) = x`). **Lector actualizado (migration 032)**: la vista incluye una nueva sección `gdpr_requests` con `pii_decrypt(gr.details_enc)`. Pendiente fase 5+ (no MVP): actualizar workflows escritores a `pii_encrypt()` directo + `DROP COLUMN question/answer/details` para retirar el plaintext.
