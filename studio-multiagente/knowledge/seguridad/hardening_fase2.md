...
3. ~~**Cifrar `client_conversations.question/answer`**~~ ✅ COMPLETO 2026-04-26.
4. ~~**Cifrar `gdpr_requests.details`**~~ ✅ COMPLETO 2026-04-26.

   **Cronología completa del cifrado PII fase 2**:
   - Migration 031: añade columnas `_enc bytea` + trigger `BEFORE INSERT/UPDATE` que auto-cifra (backward-compatible).
   - Migration 032: vista `gdpr_client_data_view` lee `pii_decrypt(_enc)` + sección nueva para `gdpr_requests`.
   - **Workflows escritores migrados (2026-04-26)**: `client_ask.Save Conversation` y `gdpr_request.Insert Request` ahora hacen `INSERT INTO ... (..., question_enc, answer_enc) VALUES (..., pii_encrypt($3), pii_encrypt($4))` directamente — el INSERT no toca la columna plain ya retirada.
   - **Migration 034**: tras migrar los escritores, `DROP TRIGGER` + `DROP FUNCTION` + `ALTER TABLE ... DROP COLUMN question, answer` y `DROP COLUMN details`. Las columnas plaintext ya no existen físicamente — un atacante con read-only access a la BD no puede recuperar PII sin la `encryption_key` de `system_config`.
   - Smoke E2E verificado: `INSERT INTO client_conversations (..., question_enc, answer_enc) VALUES (..., pii_encrypt('PII test: telefono 600111222'), pii_encrypt('Respuesta con DNI 12345678X'))` → `pii_decrypt()` recupera ambos exactos. Tamaño cifrado 94/93 bytes vs ~30 plain → confirmado que es texto cifrado, no codificación trivial.
5. **Activar RLS multi-tenant** cuando haya un segundo estudio.
6. ~~**Rotación automática de `webhook_api_key`**~~ ⚠️ Parcial 2026-04-26: workflow `cron_key_rotation_reminder` (`b7po76socKIE0WhZ`, cron mensual día 1 09:00) revisa antigüedad de `webhook_api_key`, `encryption_key`, `docusign_webhook_secret`, `signaturit_webhook_secret` y envía email a Damián con tabla coloreada cuando alguna >60d (medium) o >90d (high). La rotación misma sigue siendo manual — encryption_key requiere descifrado/recifrado y se ha decidido no automatizar para evitar lock-out por error.
7. **DKIM/SPF/DMARC** en el dominio del estudio (fuera de n8n — DNS).
8. ~~**Webhook signature** (HMAC-SHA256)~~ ✅ Hecho 2026-04-26 — sub-workflow `util_hmac_verify` (`g4k5UkQgPryA9Riz`). Implementación: usa `pgcrypto.hmac()` server-side (Task Runners de n8n bloquean `require('crypto')`). Carga el secret de `system_config` por nombre, computa HMAC del body y compara timing-safe en SQL. Soporta `sha256`/`sha512`/etc + encoding `hex` o `base64`. Smoke verificado con vector conocido (HMAC-SHA256("hello","secret") = `88aab3ed...`): caso válido → `{valid:true, reason:'ok'}`; caso inválido → `{valid:false, reason:'mismatch'}`. **Pendiente**: integrar en `contract_mark_signed` cuando se configure el proveedor de firma electrónica concreto (DocuSign/Signaturit). Hoy queda como librería lista para uso.
9. ~~**Honeypot endpoints**~~ ✅ Hecho 2026-04-26 — workflow `honeypot_trap` (`Gjsc6nxHY3KIlKj3`) con 9 paths trampa: `/admin`, `/wp-admin`, `/wp-login.php`, `/.env`, `/.git/config`, `/phpmyadmin`, `/api/v1/admin`, `/console`, `POST /login`. Cualquier hit registra `unauthorized_endpoint` con severity `high` en `security_events` y devuelve 404 genérico (sin revelar que es honeypot).

   **Auto-ban añadido (migration 033, 2026-04-26)**: nueva tabla `ip_blocklist` + funciones `is_ip_blocked(ip)` y `ban_ip(ip, duration, reason, evidence_event_id)`. El `honeypot_trap` ahora hace `Maybe Auto-Ban` después de loggear: si la misma IP ha disparado ≥3 honeypots en ventana de 10 min → `ban_ip(ip, 24h)` + raise `data_breach_suspected` severity `critical`. El `util_security_check` integra `is_ip_blocked()` en su SQL de rate-limit (sin overhead — un único query devuelve `allowed`, `reason`, `ip_blocked`). **Smoke end-to-end verificado**: 3 hits desde mi IP a /admin, /.env, /wp-admin → tras el 3º hit IP queda bannada → `client-ask` desde esa misma IP devuelve `{status:'blocked', reason:'ip_blocklist'}`. Tras unban manual, flujo normal vuelve.
