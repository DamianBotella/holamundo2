...
const ID = 'SNG5lECKgZz3Twgd';
const old = JSON.parse(await readFile(PATH, 'utf-8'));

const r = await fetch(`${N8N_API_URL}/api/v1/workflows/${ID}`, { headers: { 'X-N8N-API-KEY': N8N_API_KEY } });
...
console.log(`[sync] OK - ${live.nodes.length} nodes`);
